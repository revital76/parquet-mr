  ...
  public int run() throws IOException {
    ...
    if (encrypt) {
      ...
      if (uniform) {
        ...
        ColumnEncryptionProperties encCol = ColumnEncryptionProperties.builder("CarID", true).withKey(colKeyBytes).withKeyID("kc").build();
        columnMD.put(encCol.getPath(), encCol);
        
        byte[] colKeyBytes1 = new byte[16];
        for (byte i=0; i < 16; i++) {colKeyBytes1[i] = (byte)(i*3);}
        encCol = ColumnEncryptionProperties.builder("DriverID", true).withKey(colKeyBytes1).withKeyID("kc1").build();
        columnMD.put(encCol.getPath(), encCol);
      }
